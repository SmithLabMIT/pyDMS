...
Now that we have walked through how to use pyDMS, we provide a skeleton data file that contains all the necessary pieces to run. All that must be added is your isotherm data. This file can also be found in the pyDMS source code at:

``pyDMS/examples/skeleton.py``
...
.. code:: python
   ...
   # If you do not provide any settings, these default settings will automatically be
   # applied. The manual explains the role of each setting.
   ...
   # If you want better initial parameter ranges, you can use this helper function.
   # Note that it will overwrite any previously supplied dHD_guess, dHD_bounds,
   # dHb_guess, dHb_bounds, CH_guess, CH_bounds settings
   # dms.parameter_assist(GAS_NAME)

   # Run the code! (will save FILE_NAME.pdf and FILE_NAME.pkl)
   dms.compute(GAS_NAME, "FILE_NAME")
