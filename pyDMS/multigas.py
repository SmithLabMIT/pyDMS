...
def selectivity(*isotherms, calc=None):
    ...
    if len(isotherms) == 3 and calc is None:
        pyDMS.error_in_red('with three gases, calc must be specified as 1_numerator or 2_numerator')
    ...
    if len(isotherms) == 2:
        ...
        alpha_err = np.sqrt(alpha_err_term1+alpha_err_term2+alpha_err_term3) 
         ...
         pyDMS.error_in_red('calc="1_numerator" or "2_numerator" required for ternary selectivity')
