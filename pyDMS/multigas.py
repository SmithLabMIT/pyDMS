...
def mixed_isotherm(*gases, p_or_f, mol_frac, temp):
    ...
    for i, gas in enumerate(gases):
        if i != 0:
            ...
            term3_err_prop += (
                b0**2 * ch**2 * p0**2 * p_partial[i] ** 2 * b_err**2 / (b_terms_sum_plus_one) ** 4
            )
    ...
    term2_err_prop = (
        ch**2 * p0**2 * (1 + term2_partial_err_prop) ** 2 * b0_err**2 / (b_terms_sum_plus_one) ** 4
    )
...
def selectivity(*isotherms, calc=None):
    ...
    if len(isotherms) == 3 and calc is None:
        pyDMS.error_in_red(
            "with three gases, calc must be specified " "as 1_numerator or 2_numerator"
        )
